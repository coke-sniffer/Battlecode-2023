package iceCream.robots.carrier;

import battlecode.common.*;
import java.util.*;
import iceCream.utils.*;
import iceCream.pathing.*;
import iceCream.communication.*;
import static iceCream.RobotPlayer.OPPONENT;
import static iceCream.RobotPlayer.MY_TEAM;
...
public class Carrier {
  ...
  private static void runDepositResource(RobotController rc) throws GameActionException {
    ...
    if(rc.getHealth()<=3 && rc.getLocation().distanceSquaredTo(hqLoc)>121) {
      RobotInfo[] actionRadiusEnemyInfo = rc.senseNearbyRobots(RobotType.CARRIER.actionRadiusSquared, OPPONENT);
      for(int i = 0; i < actionRadiusEnemyInfo.length; ++i) {
        ...
        if(rc.canAttack(robotMapLocation)) {
          rc.attack(robotMapLocation);
          return;
        }
      }
    }
